...
static int triple_min(int a, int b, int c);
int NM::Editdistance::operator()(const std::string& src, const std::string& dest)
{
    int row = length(dest)  ;
    int col = length(src)  ;
    ...
    for(i = 1 , i_dest = 0 ; i <= row; i ++)
    {
        ...
        for(j = 1, j_src = 0; j <= col; j ++)
        {
            ...
            {
                src_code = (unsigned int)(src[j_src]&0xff )<< 16 | (unsigned int)(src[j_src + 1]&0xff) << 8 | (unsigned int)(src[j_src + 2]&0xff);
                j_src = j_src + 3;
            }else 
            {
    ...
    res = pp[row][col];
//    std::cout << "j_src:" << j_src <<"i_dest: " << i_dest << std::endl ;
...
static int triple_min(int a, int b, int c)//2 4 5
{
